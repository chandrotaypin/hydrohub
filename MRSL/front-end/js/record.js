/**
 * record.js — Laundry Transaction Records
 * Fetches orders with status "CLAIMED" from GET /washOrder
...
const mob = {
  ...
  totalWeight   : document.getElementById('mob-totalWeight'),
};
...
const desk = {
  ...
  totalWeight   : document.getElementById('desk-totalWeight'),
};
...
async function loadRecords() {
  ...
  try {
    ...
    // Support both { orders: [...] } and a bare array
    const raw = Array.isArray(data) ? data : (data.orders ?? data.data ?? []);

    // Endpoint already returns only CLAIMED orders
    allRecords = raw;
...
function applyFilters() {
  ...
  // Service type / bulk filter
  if (activeFilter !== 'all') {
    if (activeFilter === 'bulk') {
      result = result.filter(o => o.weightType === true);
    } else {
      result = result.filter(o =>
        (o.serviceType || '').toLowerCase() === activeFilter
      );
    }
  }
...
function buildMobCard(order, idx) {
  ...
  el.innerHTML = `
    ...
    <div class="card-row2">
      ...
      ${order.weightType ? '<span class="badge badge-bulk">⚖ Bulk</span>' : ''}
    </div>
    <div class="card-row3">
      <div class="card-meta">
        ...
        <span class="card-meta-item">
          <svg viewBox="0 0 16 16" fill="none" width="12" height="12">
            ...
            <path d="M4 14l4-6 4 6H4z" stroke="currentColor" stroke-width="1.5" fill="none"/>
          </svg>
          ${order.weight} kg
        </span>` : ''}
      </div>
...
function buildDeskRow(order, idx) {
  ...
  const payment = order.transaction
    ? capFirst((order.transaction.paymentMethod || 'cash').toLowerCase())
    : 'Cash';
...
function buildDetailContent(o) {
  ...
  const payment = o.transaction
    ? capFirst((o.transaction.paymentMethod || 'cash').toLowerCase())
    : 'Cash';
  ...
  return `
    <div class="detail-section">
      ...
      ${row('Contact',    escHtml(o.contactNumber || 'Not provided'))}
    ...
    <div class="detail-section">
      ...
      ${row('Service',    formatService(o.serviceType))}
      ${row('Weight',     o.weight ? `${o.weight} kg` : '—')}
      ${row('Bulk Load',  o.weightType ? 'Yes' : 'No')}
...
function formatService(type) {
  const map = { wash: 'Wash', wash_dry_fold: 'Wash · Dry · Fold' };
  return map[(type || '').toLowerCase()] || capFirst(type || '—');
}
...
async function deleteRecord(orderId) {
  const ok = confirm(`Delete laundry record #${orderId}? This will remove it from the database.`);
  if (!ok) return;
...
/* ── Filter pills (both rows) ── */
document.querySelectorAll('.filter-pill').forEach(btn => {
  btn.addEventListener('click', () => {
    document.querySelectorAll('.filter-pill').forEach(b => b.classList.remove('active'));
    // activate the matching pill in BOTH bars
